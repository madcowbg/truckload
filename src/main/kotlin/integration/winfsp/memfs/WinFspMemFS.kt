 ...
 */
class WinFspMemFS @JvmOverloads constructor(verbose: Boolean = false) : WinFspStubFS() {
    private val rootPath: Path = Path.of("\\").normalize()
    private val objects: MutableMap<String, MemoryObj> = HashMap()
    private val nextFileHandle: AtomicLong = AtomicLong(0)

    private var nextIndexNumber: Long = 1L
    private var volumeLabel: String = "MemFS"

    private val verboseOut: PrintStream

    init {
        objects[rootPath.toString()] = DirObj(
            null,
            rootPath,
        ...
        this.verboseOut = if (verbose) System.out else PrintStream(OutputStream.nullOutputStream())
    }
